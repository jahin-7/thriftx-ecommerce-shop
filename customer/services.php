...
if ($sort_by == 'lowToHigh') {
    $query = "SELECT * FROM products WHERE category = 'services' ORDER BY price ASC";
} else if ($sort_by == 'highToLow') {
    $query = "SELECT * FROM products WHERE category = 'services' ORDER BY price DESC";
} else {
    $query = "SELECT * FROM products WHERE category = 'services'"; // Default query if no sort is selected
}
    ...
    <script>
        function checkProductAvailability(categoryOrProduct) { 
            ...
            if (!availableItems.includes(categoryOrProduct)) {
                window.location.href = 'not-available.php';
            } else {
...
<body>
    ...
    <section class="sort-filter">
        ...
        <select id="sortBy" onchange="window.location.href='services.php?sort_by=' + this.value">
    ...
    <section class="products">
        ...
        <div class="product-list">
            <?php while ($product = $services_result->fetch_assoc()): ?>
